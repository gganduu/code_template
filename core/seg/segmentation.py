...
logger = common.get_logger('logs', name='segmentation')
...
def load_data(config):
    ...
    train_ds = dataset.CustomSegDataset(ds_dir, 'train', label_mapping, transform=transforms.Compose([
    ...
    val_ds = dataset.CustomSegDataset(ds_dir, 'val', label_mapping, transform=transforms.Compose([
        transforms.ToTensor()
    ]))
...
def train(config, device):
    ...
    for epoch in range(epochs):
        ...
        if train_epo_loss / len(train_loader.dataset) < bst_loss:
            torch.save(model, 'models/best_seg.pkl')
